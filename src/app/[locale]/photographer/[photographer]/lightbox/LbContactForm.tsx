  ...
  useEffect(() => {
    ...
    if(isOpen && boxRef){
      ...
      const focusableElements ='button, input, textarea, [tabindex]:not([tabindex="-1"])';
	    const focusableArray = Array.from(boxRef.querySelectorAll<HTMLElement>(focusableElements));
    ...
    <div className="lbcontact" ref={lightBoxRef}>
      <button type="button" className="lbcontact__overlay" onClick={onClose} aria-label={t('closeBackgroundAriaLabel')} />
			<dialog
			...
			>
				...
				>
					<Image src="/close_form.svg" width={42} height={42} alt="" />
				</button>
				{children}
			</dialog>
    </div>, document.body,
